...
<div class="col-md-12">
    <div class="card card-gray">
        ...
        <form method="POST" action="{{ route('admin.addCajas') }}" autocomplete="off">
        @csrf
        <div class="card-body">
            <div class="col-sm-10">
                <div class="form-group">
                    <label for="txt_nombre_caja">Nombre caja</label>
                    <input type="text" id="txt_nombre_caja" name="txt_nombre_caja" class="form-control text-uppercase" placeholder="Nombre de la caja" required>
                </div>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <select type="select" id="txt_name_responsable" name="txt_name_responsable" class="form-control select2 " required onchange="cargarPuesto();">
                            ...
                            @foreach($personals as $personal)
                                <option {{ old('txt_name_responsable') == $personal->id ? 'selected' : '' }} value="{{$personal->id}}">{{$personal->getFullName()}}</option>
                            @endforeach
                        </select>
                        @error('userType')
